...
int main()
{
  printf("\t\t\t----------------------- Scheduling -----------------------\n\n\n\n");
  long int n,i=0,j=0;
  printf("You can enter the process in jumbled pattern.\n\n");
  printf("A sorting algorithm will sort proceses according to Arrival Time.\n\n");
  ...
  for(i=0;i<n;i++)
  {
    printf("\nEnter Burst Time for Process [%li] : ", i+1 );
    scanf("%lf", &burstTime[i]);
    printf("Enter Arrival Time for Process [%li] : ", i+1 );
...
printf("\n\n\t\t\t -------------- Now Values are --------------\n");
  ...
  printf("\t\t\tThis program has a complexity of O(n^2)\n\n");
  return 0;
}
